...
\\
\includegraphics[width=90mm]{Boxplot_Pooled}
\\
\includegraphics[width=90mm]{Regressions_All}
\includegraphics[width=90mm]{Coefficients_All}
\includegraphics[width=90mm]{Coefficients_UIUC}
\includegraphics[width=90mm]{Coefficients_ITAM}
\includegraphics[width=90mm]{Interactive_ITAM}
\includegraphics[width=90mm]{Interactive_UIUC}
...
<<echo=False, fig=TRUE>>=
#ggplot(subdata_12.melt, aes(x = Competitiveness, y = District_Spending, color = Institution_Type)) + geom_jitter() + geom_smooth(method='lm', se = FALSE) + ggtitle("Mexican Students (ITAM)") + labs(y="District Spending", x = "Margin of Vote", color = "Electoral System") + scale_x_continuous(breaks = c(-.5, -.4, -.3, -.2, -.1, 0)) + coord_cartesian(ylim = c(4000,20000))
